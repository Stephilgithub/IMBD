...
row_number() over (partition by genre order by count(m.id) desc) as ranks from movies m
left join `genre (1)` g on m.id=g.movie_id
...
-- top 2 actors Robert coote and David niven 
...
-- It ranks actors in India based on average rating. Sanjeev is ranked 1, Gopi Krishna is ranked 2 and so on.
...
-- It gives top 5 Indian actress for hindi movies. Veena  is ranked 1, Leera kaljai is ranked 2 and so on.
