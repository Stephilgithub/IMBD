...
from movies join `genre (1)` on id=movie_id; 
...
from movies left join `genre (1)` on movie_id=id
...
join `genre (1)` g on m.id=g.movie_id
...
-- It gives top 9 directors based on no of movies. Jesse V. Johnson is ranked 1, Steven Soderbergh is ranked 2 and so on.