...
from movies m left join `genre (1)` g on g.movie_id=m.id
...
row_number() over (order by avg(avg_rating) desc) as ranks from movies
...
left join `genre (1)` g on g.movie_id=id
...
row_number() over (partition by genre order by avg(avg_rating) desc) as ranks from movies m
